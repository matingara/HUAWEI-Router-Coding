...
#
# 2Degrees - HUAWEI - code release - Alpha 0.0.1c
#
...
def main():
    ...
    print("Program to Test Netmiko Connection. Version 0.0.1a")
...
#        "password" : getpass()
        "password" : "Vx80BOgrOGki&c4IUn0#$JfqaxP"
    }

    try:
        print("TRYING -> Standard Netmiko Connect Handler")
        connectToDevice = ConnectHandler(**testDevice)
        print("Connection Result =", connectToDevice)
        print(connectToDevice.find_prompt())
    except Exception as err:
        exception_type = type(err).__name__
        print(exception_type, "\n", err)
    ...
    try:
        print("TRYING -> result_cnx = ssh_connection.disconnect()")
        result_dnx = connectToDevice.disconnect()
        print("Disconnection Successful", result_dnx)
    except Exception as err:
        exception_type = type(err).__name__
        print(exception_type, err)
    ...
    if routerInventory:    
        for thisKey in routerInventory:
            ...
            print("Querying", deviceType, "at IP address", ipAddress)
